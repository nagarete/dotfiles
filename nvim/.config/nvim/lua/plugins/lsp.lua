return {
  ...
  config = vim.schedule_wrap(function(_, opts)
    ...
    -- Autocmd to automatically install LSP servers for filetypes
    vim.api.nvim_create_autocmd("FileType", {
      callback = function()
        local filetype = vim.bo.filetype
        if filetype == "" or vim.tbl_contains({ "text", "markdown", "help" }, filetype) then
          return
        end
        ...
        -- Get lspconfig servers for filetype
        local lspconfig = require("lspconfig")
        local mason_mappings = require("mason-lspconfig.mappings")
        for lsp_name, config_func in pairs(lspconfig) do
          if type(config_func) == "function" then
            local server_config = config_func()
            if server_config.filetypes and vim.tbl_contains(server_config.filetypes, filetype) then
              local mason_package = mason_mappings.lspconfig_to_package[lsp_name]
